function Workflow_DIFCO_cell_counting_cluster_LaVision (nucleus_data_dir, Int_data_dir, threshold, area_dir, resolution, out_dir)

% Scripts for DIFCO project by Uhlen lab
% Ver 1.10 (with Resolution_adjustment)
...
% resolution example is for example, [1.2 1.2 4].
...
%This parameter is for the Lavision 6.3X (not COLM system, 10X lens). (Need to check again)
...
% [0.585 0.585 5] is resolution specific to the COLM light sheet system 
% For Lavision, resolution adjustment is added
...
cell_num = 5; % The number of cells in each cluster. The cluster which has less than cell_num won't be counted.
distance_num = 20; % The distance (micro meter) to make cells clustered.
out_dir5 = [out_dir '\' 'Point_cloud_' num2str(cell_num) 'cell_' num2str(distance_num) 'distance_pcsegdist_allcells'];
mkdir (out_dir5);
