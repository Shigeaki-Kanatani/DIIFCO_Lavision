...
% PointList has X, Y, Z coordinate (n x 3 Table)
% Parallel loop used.
...
point_number = height(PointList_space);
...
%% Save data
    ...
    Header ={'The_closest_point_of_vessel', 'Minimum_distance'};   
    ind_dists_table = array2table(ind_dists, 'VariableNames', Header);
    writetable(ind_dists_table, [out_dir '\' 'P2O-distance_detail.csv']);
...
%     writetable(T, [out_dir '\' 'F_XYZ_Space_NND.csv']);
    F_XYZ_Space_NND = cat(2, PointList_space, ind_dists_table);
    writetable(F_XYZ_Space_NND, [out_dir '\' 'F_XYZ_Space_NND.csv']);
    save([out_dir '\' 'F_XYZ_Space_NND.mat'],'F_XYZ_Space_NND');
