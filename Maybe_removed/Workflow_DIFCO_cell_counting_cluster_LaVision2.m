function Workflow_DIFCO_cell_counting_cluster_LaVision2 (nucleus_data_dir, Int_data_dir, threshold, sample_area_dir, tumor_area_dir, resolution, out_dir)
...
sample_area_image = ImageSeries_openBinary(sample_area_dir);
sample_Volume3D = length(find(sample_area_image == 1)) * resolution(1) * resolution(2) * resolution(3); 
%[0.585 0.585 5] is resolution specific to the COLM light sheet system 
%Volume of space used for analysis is calculated.

tumor_area_image = ImageSeries_openBinary(tumor_area_dir);
tumor_Volume3D = length(find(tumor_area_image == 1)) * resolution(1) * resolution(2) * resolution(3); 

Basic_summary2(F_XYZIDint_Space, F_XYZIDint_Space_ThUpper, F_XYZIDint_Space_ThLower, sample_Volume3D, tumor_Volume3D, threshold, out_dir1);
...
mkdir (out_dir2);
mean_NN_MonteCarlo_3D_pdist_LaVision(tumor_area_dir, F_XYZIDint, threshold, trial_number, resolution, out_dir2)
